...
- `phpunit.xml.dist` has conflicting `DATABASE_URL` entries and the last one wins, pointing at PostgreSQL. Tests cannot reach a real database, which is why both regression tests are attribute and container based rather than functional.
