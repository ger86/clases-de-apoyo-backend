...
- **Endpoints**: `/api/auth/{register,login,logout,password-reset}`, `/api/me` (GET and DELETE, the latter required by App Store guideline 5.1.1(v)), `/api/app-config`, `/api/search-index`, `/api/apple/transactions`, `/api/apple/notifications`.
